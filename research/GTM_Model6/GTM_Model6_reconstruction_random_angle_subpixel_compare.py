...
def fit(train,steps,device,use_sentinel,seed,aug=True):
    ...
    jitter_rng = np.random.default_rng(seed + 314159)
    ...
    views=[]
    if aug:
        # Match the built-in augmentation budget: 8 views per parent.
        # Each gets a random angle in [-45, +45] and 50% horizontal mirror.
        transforms = [
            (
        ...
        transforms = [(0.0, False)]

    for angle, mirror in transforms:
        a,y,m=augment(s,t,v,angle,mirror)

        # -------------------------------------------------
        # Sentinel-only subpixel registration jitter.
        #
        # This deliberately shifts Sentinel relative to the
        # unchanged EMIT target and validity mask.
        #
        # dx,dy are sampled in pixels from [-0.3,+0.3].
        # Bilinear interpolation is appropriate because the
        # shift is fractional.
        # -------------------------------------------------
        if use_sentinel:
            dx = float(jitter_rng.uniform(-0.3, 0.3))
            dy = float(jitter_rng.uniform(-0.3, 0.3))
