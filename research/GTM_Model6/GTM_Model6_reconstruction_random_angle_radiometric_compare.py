...
def fit(train,steps,device,use_sentinel,seed,aug=True):
    ...
    radiometric_rng = np.random.default_rng(seed + 564738)
    ...
    for angle, mirror in transforms:
        ...
        # Training-only Sentinel radiometric perturbation.
        # Each Sentinel channel receives its own gain in [0.99, 1.01].
        # EMIT target y and validity mask m are NOT changed.
        if use_sentinel:
            gains = torch.as_tensor(
                radiometric_rng.uniform(
                    0.99,
                    1.01,
                    size=a.shape[1]
                ),
