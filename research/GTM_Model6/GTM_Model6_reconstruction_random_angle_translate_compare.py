...
def fit(train,steps,device,use_sentinel,seed,aug=True):
    aug_rng = np.random.default_rng(seed + 918273)
    shift_rng = np.random.default_rng(seed + 192837)
    ...
    # Exact integer spatial translation.
    # torch.roll is used for movement, then wrapped pixels are zeroed.
    # Because the validity mask is shifted identically, introduced borders
    # are excluded from supervised loss.
    def shift_tensor(x, dy, dx):
        out = torch.roll(
            x,
            shifts=(dy, dx),
            dims=(-2, -1)
        ...
        # Remove wrap-around in Y.
        if dy > 0:
            out[..., :dy, :] = 0
        elif dy < 0:
            out[..., dy:, :] = 0

        # Remove wrap-around in X.
        if dx > 0:
            out[..., :, :dx] = 0
        elif dx < 0:
            out[..., :, dx:] = 0
    ...
    if aug:
        # Keep EXACTLY the same random-angle/mirror policy and budget
        # as the random-angle experiment.
        base_transforms = [
            (
                float(aug_rng.uniform(-45.0, 45.0)),
                bool(aug_rng.integers(0, 2))
            )
            for _ in range(8)
        ]

        # Independent integer shifts:
        # dy, dx each sampled uniformly from -4 through +4 pixels.
        shifts = [
            (
                int(shift_rng.integers(-4, 5)),
                int(shift_rng.integers(-4, 5))
            )
            for _ in range(8)
        ]
        ...
        transforms = [
            (angle, mirror, dy, dx)
            for (angle, mirror), (dy, dx)
            in zip(base_transforms, shifts)
        ]

    else:
        transforms = [(0.0, False, 0, 0)]

    for angle, mirror, dy, dx in transforms:

        # Existing rotation/mirror operation.
        a,y,m = augment(s,t,v,angle,mirror)

        # Apply the SAME translation to Sentinel, target and validity mask.
        a = shift_tensor(a, dy, dx)
        y = shift_tensor(y, dy, dx)
        m = shift_tensor(m, dy, dx)
